...
***temp code for Pakistan to replace TYPE_VISIT with US_VISIT
foreach x in 01 {       
import delimited using "$da/mnh`x'.csv", clear
rename *, upper
save "$da/mnh`x'", replace
...
***temp code for Pakistan to generate TYPE_VISIT using old visit type vars
...
***temp code for Pakistan to replace INFANTID with INFANT_ID
foreach x in 11 {       
import delimited using "$da/mnh`x'.csv", clear
rename *, upper
save "$da/mnh`x'", replace
gen INFANTID = INFANT_ID //temp code to generate INFANTID using INFANT_ID!!!
bysort MOMID PREGID INFANTID: gen dup_id=cond(_N==1,0,_n) // if no duplicate, then dup_id will be 0; if duplicate, it will be >0
*save query report
keep if dup_id > 0
keep MOMID PREGID INFANTID dup_id
