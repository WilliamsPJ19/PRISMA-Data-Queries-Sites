...
**MOM ID & PREGNANCY ID & SCREENING IDs

** Checking duplicates // we should not expect any duplicate screening IDs 
...
** Listing duplicates of SCRNID 
list SCRNID if dup_SCRNID>0, abbreviate(25) //if there are any duplicates here we need to remove them 

** Checking duplicates of MOMID 
 /* these are optional variables in these forms -- do not run if MOMID or PREGID are not available */
...
list MOMID PREGID if dup_MOMID>0, abbreviate(25)  //if there are any duplicates here we need to remove them 
